defmodule Kaufmann.Stages.ProducerTest do
  ...
  setup do
    {:ok, _} = Kaufmann.Stages.Producer.start_link([])
  ...
  describe "handle_demand" do
    test "when no messages and demand > 0" do
      ...
      res = Kaufmann.Stages.Producer.handle_demand(10, %{message_set: [], demand: 10})
      assert res == {:noreply, [], %{demand: 10, message_set: []}}
    end

    test "when messages > demand && demand > 0" do
      res =
        Kaufmann.Stages.Producer.handle_demand(5, %{message_set: [1, 2, 3, 4, 5, 6], demand: 5})
    ...
    test "when messages and 0 demand" do
      res = Kaufmann.Stages.Producer.handle_demand(0, %{message_set: [1, 2, 3, 4, 5], demand: 10})
      assert res == {:noreply, [], %{demand: 0, message_set: [1, 2, 3, 4, 5]}}
    end

    test "when no messages, and  0 demand" do
      res = Kaufmann.Stages.Producer.handle_demand(0, %{message_set: [], demand: 10})
    ...
    test "When demand changes over multiple calls" do
      ...
      {reply, next_message_set, state} =
        Kaufmann.Stages.Producer.handle_demand(5, %{
          message_set: [1, 2, 3, 4, 5, 6],
          demand: 5,
          from: from
        })

      {reply, next_message_set, state} = Kaufmann.Stages.Producer.handle_demand(0, state)
      ...
      {reply, next_message_set, state} = Kaufmann.Stages.Producer.handle_demand(1, state)
