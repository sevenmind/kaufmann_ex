defmodule KafkaExGenStageConsumer.Supervisor do
  @moduledoc """
  ...
  options as `KafkaExGenStageConsumer.start_link/5`.
  ...
  @spec start_link(
          ...
          KafkaExGenStageConsumer.options()
        ) :: Elixir.Supervisor.on_start()
  def start_link(subscriber_module, group_name, assignments, opts \\ []) do
    start_link_result =
      Elixir.Supervisor.start_link(
        __MODULE__,
        {subscriber_module, group_name, assignments, opts}
      )
  ...
  def init({subscriber_module, group_name, _assignments, _opts}) do
    children = [
      worker(KafkaExGenStageConsumer, [subscriber_module, group_name])
    ]
