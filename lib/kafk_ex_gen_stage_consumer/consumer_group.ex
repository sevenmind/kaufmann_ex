defmodule KafkaExGenStageConsumer.ConsumerGroup do
  @moduledoc """
  ...
  **Note** It is not necessary for the Elixir nodes in a consumer group to be
  ...
  alias KafkaExGenStageConsumer.ConsumerGroup
  alias KafkaEx.ConsumerGroup.PartitionAssignment
  alias KafkaExGenStageConsumer, as: GenConsumer
  ...
  *Note* When starting a consumer group with multiple topics, you should
  ...
  def start_link(subscribing_module, group_name, topics, opts \\ []) do
    {supervisor_opts, module_opts} =
      Keyword.split(opts, [:name, :strategy, :max_restarts, :max_seconds])

    Supervisor.start_link(
      __MODULE__,
      {subscribing_module, group_name, topics, module_opts},
      supervisor_opts
    )
  end
  ...
  def get_manager_pid(supervisor_pid) do
    {_, pid, _, _} =
      Enum.find(
        ...
        fn
          {KafkaExGenStageConsumer.ConsumerGroup.Manager, _, _, _} -> true
          {_, _, _, _} -> false
        end
      )
  ...
  @doc false
  def start_consumer(pid, subscribing_module, group_name, assignments, opts) do
    child =
      supervisor(
        KafkaExGenStageConsumer.Supervisor,
        [subscribing_module, group_name, assignments, opts],
        id: :consumer
      )
  ...
  @doc false
  def init({subscribing_module, group_name, topics, opts}) do
    opts = Keyword.put(opts, :supervisor_pid, self())

    children = [
      worker(
        KafkaExGenStageConsumer.ConsumerGroup.Manager,
        [subscribing_module, group_name, topics, opts]
      )
    ]
